...
const { BOT_TOKEN_PLANT } = process.env;
const { TOKEN_GOOGLE } = process.env;
const { TOKEN_GOOGLE_ID } = process.env;
...
bot.on('photo', async (msg) => {
  const chatId = msg.chat.id;
  bot.getFile(msg.photo[1].file_id).then((link) => {
        ...
        (response) => {
          ...
          fetch('https://api.plant.id/v2/identify', {
            ...
            .then(async (data) => {
              ...
              const urlGoogle = encodeURI(`https://www.googleapis.com/customsearch/v1?key=${TOKEN_GOOGLE}&cx=${TOKEN_GOOGLE_ID}&q=${resultPlantNew}+уход`);
              fetch(urlGoogle)
                .then((response) => response.json())
